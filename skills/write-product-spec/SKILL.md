...
description: Write a PRODUCT.md spec for a significant feature, focused on detailed behavior and validation. Use when the user asks for a product spec, desired behavior doc, or PRD, wants to define feature behavior before implementation, or when the feature is substantial or behaviorally ambiguous enough that a written spec would improve implementation or review.
---
...
"User" is not limited to the end user of an app. It means whoever consumes the surface being designed:
...
Write specs to `specs/<feature-slug>/PRODUCT.md`, where `<feature-slug>` is a short, stable kebab-case name (for example, `specs/vertical-tabs-hover-sidecar/PRODUCT.md`).

`specs/` should contain only logical feature-slug directories as direct children — no engineer-named or externally-tracked-work-named subdirectories.

## Before writing

Gather only the context you need: logical feature slug and feature name, feature summary, target users, key behaviors, edge cases, and how the feature will be validated. Ask focused questions for missing context rather than guessing.

### Visual references

If the feature has UI or interaction design, ask for reference images or a written visual concept before drafting the Behavior section. They can establish visual states, spacing, hierarchy, and edge-case layouts that would otherwise be guessed.

- If the user provides images, links, or a visual concept, include them under a short `## Visual references` section (or inline near the top of Behavior).
- If the user confirms no visual reference exists, note `Visual references: none provided` so the absence is explicit rather than ambiguous.
...
- **Visual references** — Include supplied reference images, links, or visual concepts when design matters, or an explicit `Visual references: none provided` note when none exists. Omit entirely for non-visual features. See "Visual references" above.
...
14. Sharing or exporting a block (share link, export, save as file) preserves the original markdown source, not the rendered form.

15. Theming: table borders, header backgrounds, alternating row shading (if any), and link/code styles all come from the active product theme. No hard-coded colors.
