...
Write specs to `specs/<feature-slug>/TECH.md`, where `<feature-slug>` is a short, stable kebab-case name (for example, `specs/vertical-tabs-hover-sidecar/TECH.md`). Match the slug used by the sibling `PRODUCT.md` when one exists. `specs/` should contain only logical feature-slug directories as direct children.
...
1. **Context** — What's being built, how the current system works in the area being changed, and the most relevant files with line references. Combine the "problem," "current state," and "relevant code" into one grounded section. Example references:
   - [`path/to/entrypoint.ext:42 @ <commit-sha>`](https://github.com/<owner>/<repo>/blob/<commit-sha>/path/to/entrypoint.ext#L42) — entry point for the user flow
   - [`path/to/module.ext (120-220) @ <commit-sha>`](https://github.com/<owner>/<repo>/blob/<commit-sha>/path/to/module.ext#L120-L220) — state and event handling that will likely change
...
4. **Parallelization** — Actively evaluate whether parallel sub-agents (launched through the available subagent mechanism) would meaningfully reduce wall-clock time or isolate work. Skip this section if no subagent mechanism is available. When the spec proposes using sub-agents, include for each proposed agent:
