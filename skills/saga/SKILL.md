---
name: saga
description: Run an autonomous, spec-driven development "saga" for medium-to-large features using an orchestrator and independently scoped worker agents when useful. Use when the user invokes /saga, asks to autonomously build a sizable feature end-to-end with minimal human intervention, wants a comprehensive spec broken into milestones and tasks with airtight validation criteria before parallelized implementation, or wants coordinated subagents while preserving context. Also use when asked to continue, resume, or pick up an existing saga from its durable state location.
---

# Saga

Saga is an autonomous, spec-driven development workflow for **medium-to-large features** that should be implemented mostly without human intervention, except at a few discrete touch points. You act as the **orchestrator**: you turn a rough prompt into an airtight spec, then delegate only independent implementation work while keeping your own context window clean.
...
- **Protect the orchestrator's context.** You are the long-lived coordinator. Delegate heavy reading, research, or implementation only when isolation or parallelism materially helps; receive compact reports back. Keep state in durable storage so your understanding survives compaction and you can re-read rather than re-hold.
...
When running in Amp, load `harnesses/amp.md` before choosing state storage or delegating work.

## Saga state

Store each saga in durable storage that survives the intended executor lifecycle. For a local long-running workflow, a directory outside the repository is appropriate; for ephemeral or remote executors, use a committed handoff, a durable branch, or host-provided persistent thread/workflow state. Name it from the feature slug plus a timestamp and record its location as the saga's stable identity.
...
When asking the user anything in this phase, provide concrete options (single- or multi-select) rather than open-ended questions. Identify a recommended option when there is a sensible default. Open-ended prose questions slow the user down and invite vague answers; options force crisp decisions.

### 1. Intake and frame

Restate the request as a one-paragraph problem statement and the rough shape of the feature. Identify the major unknowns you will need to close. Pick a durable saga state location (feature slug + timestamp), confirm it with the user, and create it; everything below is written there.
...
Present the saga spec — walk the user through `SAGA.md` and the milestone/task specs — and ask them to approve or request changes. **Do not begin Phase 2 until the user approves.** This is the primary human checkpoint.
...
- **Delegate when it helps.** Use the host's subagent mechanism only for independently specifiable work or bounded research that would otherwise crowd coordination context. Keep tightly coupled or small work in the main flow.
- **Batch by parallelism.** Within a milestone, launch all independent tasks together with shared context and task-specific instructions. Run dependent milestones in sequence. Use a Mermaid/DAG mental model from the task dependencies.
...
Give workers shared rules (repository, base branch, toolchain, coding standards, validation method, and reporting format) plus a task-specific brief. Instruct every worker to:
...
1. Launch only the milestone's genuinely parallelizable tasks as workers. Record each durable worker handle, branch, or handoff reference in `PROGRESS.md` alongside its task and worktree when applicable.
...
3. **Loop in the user for manual acceptance:** accept, or report specific issues. If they report issues, capture them as new tasks, run a focused Phase 2 mini-loop (delegate → self-validate → integrate), and re-present. Repeat until the user accepts.
