...
Use this when asked to continue, resume, or pick up a saga — typically a fresh orchestrator with no memory of the earlier session. The durable saga state is the contract and `PROGRESS.md` is the current state. Your job is to reconstruct where the saga stands, reconcile it against reality, and resume the Phase 2/3 loop from `SKILL.md` — without re-reading the whole tree into context.

## 1. Locate the saga state

Use the recorded state location, supplied path, durable branch, or host-provided workflow/thread reference. If the user named it or gave a path, use that. Otherwise inspect the configured state root and ask the user to choose if ambiguous. The feature slug plus timestamp is the saga's stable identity.
...
- If a blocker or open question recorded in `PROGRESS.md` needs the user, resolve it first with focused options.
