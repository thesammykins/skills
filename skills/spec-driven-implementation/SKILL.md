...
- `specs/<feature-slug>/PRODUCT.md`
- `specs/<feature-slug>/TECH.md`

For example:

- `specs/example-feature/PRODUCT.md`
- `specs/example-feature/TECH.md`

`specs/` should contain only logical kebab-case feature-slug directories as direct children. Choose a short, stable slug from the feature name (for example, `vertical-tabs`) and use the same slug for its product and technical specs.

Keep external tracking separate from the spec directory. The feature slug is the spec's stable identity.
...
If the feature has UI or interaction design, ask for reference images or a written visual concept if available. If none exists, continue but call that out explicitly in the product spec.

Use the logical feature slug consistently in the spec directory and cross-references.
...
- recorded walkthroughs or equivalent feature demonstrations when appropriate
