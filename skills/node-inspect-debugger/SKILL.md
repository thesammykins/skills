...
- **`ndb` / CDP via `chrome-remote-interface`** — scriptable from Node/Python; best when you want to automate many breakpoints, collect state across runs, or debug non-interactively.
...
- A CLI/React/Ink app crashes or behaves wrong and you want to inspect pre-render state
...
If `chrome-remote-interface` isn't in your project, install to a throwaway location:
...
## Running Vitest / Jest Tests Under the Debugger

```bash
# Vitest: run a single test file paused on entry
node --inspect-brk ./node_modules/vitest/vitest.mjs run --no-file-parallelism src/foo.test.tsx
```

In another terminal: `node inspect -p <pid>`, then `sb('src/foo.tsx', 42)`, `cont`.
...
6. **Security.** `--inspect=0.0.0.0:9229` exposes arbitrary code execution. Always bind to `127.0.0.1` (the default) unless you have an isolated network.
